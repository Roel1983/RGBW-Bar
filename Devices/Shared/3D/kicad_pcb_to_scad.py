...
with open(args.outputfile, 'w') as scad_file:
	...
	if pcb_thickness:
		scad_file.write("PCB_THICKNESS_{} = {};\n".format(
			args.boardname.upper(),
			pcb_thickness))
	else:
		print("Error: No PCB_THICKNESS_{} detected".format(
			args.boardname.upper()));
		exit_code = 1;
	if pcb_bounds:
	...
	for component in components:
		...
		scad_file.write('\t["{}", [{}, {}], {}],\n'.format(
			...
			component["at"][2]))
		scad_file.write('\t["{}", "{}"]];\n'.format(
			re.match("(.*?):(.*)", component["foot_print"]).group(1),
			re.match("(.*?):(.*)", component["foot_print"]).group(2)))
		scad_file.write('\n')
	if len(components) == 0:
		print("Error: No components detected");
		exit_code = 3;

	scad_file.write('ALL_COMPONENTS_{} = ['.format(args.boardname.upper()))
	is_first = True
	length   = 1000;
	for component in components:
		if is_first:
